...
3. **Diretiva `go` do `go.mod`: `go 1.22`.** O piso do ADR 0001. O toolchain instalado (1.27.1) é compatível com essa diretiva; ela **não** sobe para a versão do toolchain, para não fechar a porta a quem tem Go mais antigo. Task 2.1.
...
6. **Portas do Compose.** Postgres publicado no host em `5432`; `serve` publicado em `8080`. Tasks 4.3 e 6.2. Se colidir com um Postgres já instalado na máquina, o ajuste é local (`docker compose` com override), não uma mudança do plano.
