...
- [x] 8.1 Escrever `.github/workflows/ci.yml` rodando, em push e PR, `go vet ./...`, `staticcheck ./...`, `go test -race ./...` **e `make eval-resolve`** — os quatro passos que `testing.md` §5 exige em todo PR —, com cache de módulos e **sem** `eval-parse` (que chama serviço externo pago e só roda quando muda prompt ou modelo). O passo `make eval-resolve` é hoje um no-op verde (task 6.5) e existe para que o eixo determinístico de resolução já esteja encanado no CI quando os cenários chegarem no M2. **Depende de 6.5.** Verificação: os quatro comandos passam localmente na mesma ordem; o workflow fica verde no primeiro push com PR aberto; `grep -c eval-parse .github/workflows/ci.yml` retorna `0`. Commit: `ci: vet, staticcheck, testes com -race, eval-resolve e govulncheck em todo PR`. (owner: agent) *Acréscimo ao plano original (2026-09-19): passo bloqueante `govulncheck ./...`, com a spec `local-environment` ajustada (requirement e cenários). Motivo: o `go 1.22` fixado escondia versões com vulnerabilidades que o código chamava (GO-2026-5004, GO-2026-5970) e só foram achadas por acaso; sem o passo isso voltaria em silêncio. Ferramentas com versão fixa (`staticcheck` 2026.2.1, `govulncheck` v1.8.0), actions fixadas por SHA (`checkout` v7.0.1, `setup-go` v7.0.0). O CI usa o último patch `1.26.x` derivado do `go.mod`, não o `1.26.0` exato: com 1.26.0 o `govulncheck` acusou 16 vulnerabilidades da stdlib, todas corrigidas em 1.26.1+. **Ressalva de verificação: a execução real no GitHub Actions NÃO foi verificada** (não houve push, que o usuário não autorizou); o critério "verde no primeiro push com PR aberto" continua **pendente**. Foram verificados só localmente: `actionlint` limpo e os cinco comandos na mesma ordem, inclusive com o Go 1.26.8.*
- [x] 8.2 Atualizar a seção "Rodando localmente" do `README.md` com os comandos reais (`cp .env.example .env`, `docker compose up`, `make migrate`, `make test`), citando as portas `8080` (serve) e `5432` (Postgres) e a nota do `PATH` do Go, e manter M1 como ⬜ — o scaffolding não é o *walking skeleton*. Verificação: leitura humana; todo comando citado foi executado com sucesso em alguma task anterior. Commit: `docs(readme): instruções de execução local`. (owner: human) *Redigida pelo Claude e revisada e aprovada pelo usuário em 2026-09-21.*
